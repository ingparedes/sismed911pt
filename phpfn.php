	...
	public function validateUser(&$usr, &$pwd, $autologin, $provider = "")
	{
		...
		if (!$valid) {
			...
			if ($valid) {
				...
				$this->setCurrentUserName($Language->phrase("UserAdministrator")); // System Administrator
...
class Language
{
	...
	function Language_Load() {
		...
		// Example:
		$this->setPhrase("en", "English");
		$this->setPhrase("es", "Español");
		$this->setPhrase("pt", "Português");
		$this->setPhrase("fr", "Français"); // Refer to language file for the actual phrase id
		$this->Type = "DROPDOWN";
	}
}
...
function SetupLoginStatus() {
	...
	if ($currentPage != $changePasswordPage) {
		...
		if (!IsMobile())
			$changePasswordUrl = "return ew.modalDialogShow({lnk:this,btn:'ChangePwd',caption:ew.language.phrase('ChangePwd'),size:'',url:'" . HtmlEncode(GetUrl($changePasswordPage)) . "'});";
	}
