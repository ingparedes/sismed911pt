$(function () {
	...
	var tableMaestro = $('#tableMaestro').DataTable({
		...
		columnDefs: [
			...
			{
				render: function (data, type, row) {
					return row.estado_llamada == 1 ? 'Interhospitalario' : 'Prehospitalario';
				},
				targets: 3,
			},
			{
				render: function (data, type, row) {
					return row.nombre1 ? row.nombre1 : '' + ' ' + row.nombre2 ? row.nombre2 : '' + ' ' + row.apellido1 ? row.apellido1 : '' + ' ' + row.apellido2 ? row.apellido2 : '';
				},
				targets: 4,
			},
			{
			...
			{
				render: function (data, type, row) {
					return '<button ' + (row.accion == 2 ? 'hidden' : '') + ' type="button" class="btn btn-light" data-toggle="modal" data-target="#modal-dispatch"><i class="fa fa-ambulance" aria-hidden="true"></i></button>';
				},
				targets: 13,
			},
		],
		...
		if (type === 'row') {
			...
			$.ajax({
				...
				.done(function (data) {
					var name = '';
					$('#p_ide').empty();
					$('#p_ide').append($("<option value='0'>" + language['select'] + '</option>'));
					$.each(data['ide'], function (index, value) {
						...
						$('#p_ide').append($("<option value='" + value.id_tipo + "'>" + name + '</option>'));
						if (dataSelect.tipo_doc == value.id_tipo) {
							$('#p_ide option[value=' + value.id_tipo + ']').attr('selected', true);
							changeIDE(false);
						}
					});

					$('#p_typeage').empty();
					$('#p_typeage').append($("<option value='0'>" + language['select'] + '</option>'));
					$.each(data['age'], function (index, value) {
						...
						$('#p_typeage').append($("<option value='" + value.id_edad + "'>" + name + '</option>'));
						if (dataSelect.cod_edad == value.id_edad) {
							$('#p_typeage option[value=' + value.id_edad + ']').attr('selected', true);
						}
					});

					$('#ec_triage').empty();
					$('#ec_triage').append($("<option value='0' style='color: initial'>" + language['select'] + '</option>'));
					var color = '';
					$.each(data['triage'], function (index, value) {
						...
						$('#ec_triage').append($("<option value='" + value.id_triage + "'class='fa' style='color: " + color + "'>&#xf0c8; " + name + '</option>'));
						if (dataSelect.triage == value.id_triage) $('#ec_triage option[value=' + value.id_triage + ']').attr('selected', true);
						paintSelect();

						$('#ec_type').empty();
						$('#ec_type').append($("<option value='0'>" + language['select'] + '</option>'));
						$.each(data['type'], function (index, value) {
							...
							$('#ec_type').append($("<option value='" + value.id_tipopaciente + "'>" + name + '</option>'));
							if (dataSelect.id_tipopaciente == value.id_tipopaciente) {
								$('#ec_type option[value=' + value.id_tipopaciente + ']').attr('selected', true);
							}
						});
					});
				})
			...
			if (dataSelect.hospital_destino) $('#hosp_dest').val(dataSelect.hospital_destino + ' ' + dataSelect.nombre_hospital);
		...
		if ($('#p_ide option:selected').val() != 0) {
			if ($('#p_ide option:selected').val() == 1) {
				if (number_validate($('#p_number').val())) {
					...
					if (crud) crud_ajax('tipo_doc', $('#p_ide option:selected').val(), 'updateP');
				}
			} else {
	...
	$('#p_typeage').on('change', function () {
		if ($('#p_typeage option:selected').val() != 0) crud_ajax('cod_edad', $('#p_typeage option:selected').val(), 'updateP');
	});
	...
	$('.gender').on('click', function () {
		if (
			!dataSelect.genero ||
			(dataSelect.genero == 1 && ($('input:checked').val() == 2 || $('input:checked').val() == 3)) ||
			(dataSelect.genero == 2 && ($('input:checked').val() == 1 || $('input:checked').val() == 3)) ||
			(dataSelect.genero == 3 && ($('input:checked').val() == 1 || $('input:checked').val() == 2))
		)
	...
	$('#ec_triage').on('change', function () {
		paintSelect();
		if ($('#ec_triage option:selected').val() != 0) crud_ajax('triage', $('#ec_triage option:selected').val(), 'updatePrehEC');
	});

	$('#ec_type').on('change', function () {
		if ($('#ec_type option:selected').val() != 0) crud_ajax('tipo_paciente', $('#ec_type option:selected').val(), 'updatePrehEC');
	});
	...
	$('#modalR').on('show.bs.modal', function () {
		...
		$.ajax({
			...
			.done(function (data) {
				$('#selectRazon').empty();
				$('#selectRazon').append($("<option value='0'>" + language['select'] + '</option>'));
				$.each(data, function (index, value) {
					$('#selectRazon').append($("<option value='" + value.id_tranlado_fallido + "'>" + value.tipo_cierrecaso_es + '</option>'));
				});
			})
			.fail(function () {
				console.log('error');
	...
	function number_validate(num) {
		...
		numberValidate ? $('.form-control#p_number').removeClass('is-invalid') : $('.form-control#p_number').addClass('is-invalid');
