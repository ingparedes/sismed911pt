$(function () {
  ...
  tableAdmission.on('select', function (e, dt, type, indexes) {
    ...
    $.ajax({
      ...
      method: 'POST',
      data: {
        option: 'updateFechaInicioClasificacion',
        data: dataSelect.id_admision,
      },
    })
      .done(function () {
        ...
        switch (dataSelect.id_motivoatencion) {
          case '1':
            ...
            $(
              '#causeTrauma option[value=' + dataSelect.id_causatrauma + ']'
            ).attr('selected', true);
            break;
          case '2':
        ...
        if (dataSelect.dolor)
          $('#dolor option[value=' + dataSelect.dolor + ']').attr(
            'selected',
            true
          );
        switch (language['language']) {
