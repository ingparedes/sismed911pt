...
class setData extends DB
{
    ...
    function setPaciente($jsonParam)
    {
        ...
        $prioridad = $jsonParam['prioridad'];
        $incidente = $jsonParam['incidente'];
        $direcevento = $jsonParam['direcevento'];
        $query = $this->connect()->query("SELECT * FROM public.pacientegeneral WHERE cod_casointerh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = "INSERT INTO public.pacientegeneral (
                    /* BLOQUE */ cod_casointerh,num_doc,tipo_doc, 
            ...
            $sql = "UPDATE public.preh_maestro SET direccion = ?, incidente = ?, prioridad = ? WHERE cod_casopreh = ? ";
            $query = $this->connect()->prepare($sql)->execute([$direcevento, $incidente, $prioridad, $cod_caso]);
            $sql = "UPDATE public.preh_servicio_ambulancia SET k_inical = ? WHERE cod_casopreh = ? ";
    ...
    function setDiagnostico($jsonParam)
    {
        ...
        $query = $this->connect()->query("SELECT * FROM public.preh_evaluacionclinica WHERE cod_casopreh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = 'INSERT INTO public.preh_evaluacionclinica (cod_casopreh,cod_paciente,triage,fecha_horaevaluacion, sv_tx, sv_fc, sv_fr, sv_temp, sv_sato2, sv_gl, cod_diag_cie, ap_diabetes, ap_cardiop, ap_convul, ap_asma, ap_acv, ap_has, ap_alergia, ap_med_paciente, ap_otros) VALUES (?,(select id_paciente from pacientegeneral where cod_casointerh = ?),?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?);';
            ...
            $sql = 'UPDATE public.preh_evaluacionclinica SET fecha_horaevaluacion = ?, triage = ?, sv_tx = ?, sv_fc = ?, sv_fr = ?, sv_temp = ?, sv_sato2 = ?, sv_gl = ?,  cod_diag_cie = ?, ap_diabetes = ?, ap_cardiop = ?, ap_convul = ?, ap_asma = ?, ap_acv = ?, ap_has = ?, ap_alergia = ?, ap_med_paciente = ?, ap_otros = ? WHERE cod_casopreh = ?;';
    ...
    function setOthers($jsonParam)
    {
        ...
        $atenfermero = $jsonParam['atenfermero'];
        $atmedico = $jsonParam['atmedico'];
        $casomuerte = $jsonParam['casomuerte'];
        $sql = "UPDATE public.ambulancias SET estado = 1 WHERE cod_ambulancias = ?;";
        $query = $this->connect()->prepare($sql)->execute([$codambu]);
        $query = $this->connect()->query("SELECT * FROM public.preh_maestro WHERE cod_casopreh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $query = $this->connect()->query("");
            $query = $this->connect()->query("SELECT 'Guardado Correctamente' as Respuesta;");
        } else {
            $sql = "UPDATE public.preh_servicio_ambulancia SET k_final = ?, medico = ?, paramedico = ?, cuando_murio = ? WHERE cod_casopreh = ?;";
            $query = $this->connect()->prepare($sql)->execute([$kfinal, $atmedico, $atenfermero, $casomuerte, $cod_caso]);
    ...
    function setTraumaFisico($jsonParam)
    {
        ...
        $posy = $jsonParam['posy'];
        $pos = $jsonParam['pos'];
        $query = $this->connect()->query("SELECT * FROM public.explo_fisica_registro WHERE cod_casopreh = '" . $cod_caso . "' AND pos = '" . $pos . "'");
        if ($query->rowCount() == 0) {
    ...
    function setHoraI($jsonParam)
    {
        ...
        if ($query->rowCount() == 0) {
            ...
            $sql = "UPDATE public.preh_servicio_ambulancia SET hora_inicio = ? WHERE cod_casopreh = ?;";
    ...
    function setHoraE($jsonParam)
    {
        $cod_caso = $jsonParam['cod_caso'];
        $hora = $jsonParam['hora'];
        $query = $this->connect()->query("SELECT * FROM public.preh_servicio_ambulancia WHERE cod_casopreh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $query = $this->connect()->query("");
            $query = $this->connect()->query("SELECT 'Guardado Correctamente' as Respuesta;");
        } else {
            $sql = "UPDATE public.preh_servicio_ambulancia SET hora_llegada = ? WHERE cod_casopreh = ?;";
            $query = $this->connect()->prepare($sql)->execute([$hora, $cod_caso]);
            $query = $this->connect()->query("SELECT 'Actualizado Correctamente' as Respuesta;");
        }
        return $query;
    ...
    function setHoraH($jsonParam)
    {
        ...
        if ($query->rowCount() == 0) {
            ...
            $sql = "UPDATE public.preh_servicio_ambulancia SET hora_destino = ? WHERE cod_casopreh = ?;";
            $query = $this->connect()->prepare($sql)->execute([$hora, $cod_caso]);
            $query = $this->connect()->query("SELECT 'Actualizado Correctamente' as Respuesta;");
        }
        return $query;
    ...
    function setHoraB($jsonParam)
    {
        ...
        if ($query->rowCount() == 0) {
            ...
            $sql = "UPDATE public.preh_servicio_ambulancia SET hora_base = ? WHERE cod_casopreh = ?;";
            $query = $this->connect()->prepare($sql)->execute([$hora, $cod_caso]);
            $query = $this->connect()->query("SELECT 'Actualizado Correctamente' as Respuesta;");
        }
        return $query;
    ...
    function SynchLocal($jsonParam)
    {
        ...
        $direcevento = $jsonParam['infobasica']['direcevento'];
        $incidente = $jsonParam['infobasica']['incidente'];
        $prioridad = $jsonParam['infobasica']['prioridad'];
        ...
        $atenfermero = $jsonParam['otros']['atenfermero'];
        $atmedico = $jsonParam['otros']['atmedico'];
        $latitud = $jsonParam['latitud'];
        $longitud = $jsonParam['longitud'];
        ...
        $hora_a = $jsonParam['hora_a'];
        $posx = $jsonParam['firmas']['posx'];
        $posy = $jsonParam['firmas']['posy'];
        $ancho = $jsonParam['firmas']['ancho'];
        $query = $this->connect()->query("SELECT * FROM public.preh_maestro WHERE cod_alfa = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = "INSERT INTO public.preh_maestro (fecha,descripcion,nombre_confirma,telefono_confirma,hospital_destino,nombre_medico,observacion,cierre,longitud,latitud,cod_alfa,direccion,incidente,prioridad) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING cod_casopreh";
            $query = $this->connect()->prepare($sql)->execute([$hora_a, $desc, $nombre_confirma, $telefono_confirma, $hospital, $med, $obscaso, $cierre, $longitud, $latitud, $cod_caso, $direcevento, $incidente,$prioridad]);
        } else {
            $sql = "UPDATE public.preh_maestro SET fecha = ?,descripcion = ?, nombre_confirma = ?, telefono_confirma = ?, hospital_destino = ?, nombre_medico = ?, observacion = ?, cierre = ?, longitud = ?, latitud = ?,direccion = ?,incidente = ?, prioridad = ? WHERE cod_alfa = ?  RETURNING cod_casopreh;";
            $query = $this->connect()->prepare($sql)->execute([$hora_a, $desc, $nombre_confirma, $telefono_confirma, $hospital, $med, $obscaso, $cierre, $longitud, $latitud, $cod_caso, $direcevento, $incidente,$prioridad]);
        }

        $dbconn3 = pg_connect("host=190.85.49.114 port=12750 dbname=sismed_911 user=dba_bitcode password=z*8hE5R&v9ib");
        $res = pg_query($dbconn3, "SELECT cod_casopreh FROM preh_maestro order by cod_casopreh desc limit 1");
        $cod_caso = pg_fetch_result($res, 0, 0);

        $query = $this->connect()->query("SELECT * FROM public.pacientegeneral WHERE cod_casointerh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = "INSERT INTO public.pacientegeneral ( cod_casointerh,id_paciente,num_doc,tipo_doc,nombre1,nombre2,apellido1,apellido2,genero,edad,fecha_nacido,cod_edad,telefono,celular,direccion,aseguradro,prehospitalario ) VALUES ( ?, COALESCE(( SELECT MAX (id_paciente) FROM pacientegeneral),0)+1 ,?,?, ?, ? ,?,?, ?, ? ,?,?,?, ? ,?,?,'1');";
            $query = $this->connect()->prepare($sql)->execute([$cod_caso, $num_doc, $tipo_doc, $nombre1, $nombre2, $apellido1, $apellido2, $genero, $edad, $fecha_nacido, $cod_edad, $telefono, $celular, $direccion, $asegurador]);
        } else {
            $sql = "UPDATE public.pacientegeneral SET num_doc = ?, tipo_doc = ?, nombre1 = ?, nombre2 = ?, apellido1 = ?, apellido2 = ?, genero = ?, edad =  ?, fecha_nacido = ?, cod_edad = ?, telefono = ?, celular =  ?, direccion = ?, aseguradro = ? WHERE cod_casointerh = ?;";
            $query = $this->connect()->prepare($sql)->execute([$num_doc, $tipo_doc, $nombre1, $nombre2, $apellido1, $apellido2, $genero, $edad, $fecha_nacido, $cod_edad, $telefono, $celular, $direccion, $asegurador, $cod_caso]);
        }
        $query = $this->connect()->query("SELECT * FROM public.causa_registro WHERE cod_casopreh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = "INSERT INTO public.causa_registro (cod_casopreh,id_causa) VALUES (?,?);";
        ...
        $sql = "DELETE FROM public.procedimiento_registro WHERE cod_casopreh = ?;";
        $query = $this->connect()->prepare($sql)->execute([$cod_caso]);
        $sql = "DELETE FROM public.insumos_registros WHERE cod_casopreh = ?;";
        $query = $this->connect()->prepare($sql)->execute([$cod_caso]);
        $sql = "DELETE FROM public.medicamentos_registros WHERE cod_casopreh = ?;";
        $query = $this->connect()->prepare($sql)->execute([$cod_caso]);
        $sql = "DELETE FROM public.explo_fisica_registro WHERE cod_casopreh = ?;";
        $query = $this->connect()->prepare($sql)->execute([$cod_caso]);
        $sql = "DELETE FROM public.explo_general_registro WHERE cod_casopreh = ?;";
        $query = $this->connect()->prepare($sql)->execute([$cod_caso]);
        ...
        $query = $this->connect()->query("SELECT * FROM public.preh_servicio_ambulancia WHERE cod_casopreh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = "INSERT INTO public.preh_servicio_ambulancia (cod_casopreh,cod_ambulancia,hora_asigna,hora_llegada,hora_inicio,hora_destino,hora_preposicion,observaciones,k_final,k_inical,medico,paramedico) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
            $query = $this->connect()->prepare($sql)->execute([$cod_caso, $cod_ambu, $hora_a, $hora_e, $hora_i, $hora_h, $hora_b, $obscaso, $kfinal, $kinicial,$atmedico,$atenfermero]);
        } else {
            $sql = "UPDATE public.preh_servicio_ambulancia SET k_final = ?, k_inical = ?, hora_asigna = ?, hora_inicio = ?, hora_llegada = ?, hora_preposicion= ?, hora_destino= ? WHERE cod_casopreh = ?;";
            $query = $this->connect()->prepare($sql)->execute([$kfinal, $kinicial, $hora_a, $hora_i, $hora_e, $hora_b, $hora_h, $cod_caso]);
        }
        $query = $this->connect()->query("SELECT * FROM public.firmas_registro WHERE cod_casopreh = '" . $cod_caso . "'");
        if ($query->rowCount() == 0) {
            $sql = "INSERT INTO public.firmas_registro (cod_casopreh,posx,posy,ancho) VALUES (?,?,?,?);";
