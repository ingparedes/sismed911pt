    ...
    if ($data['Token'] === 'MAgJKq3LaKxL3pae') {
        switch ($data['Servicio']) {
            ...
            case 'Hospitales':
                ...
                break;
            case 'Casos Cerrados':
                $api->getCasosCerrados();
                break;
            case 'Caso PDF':
                $api->getCasoPDF($data['Caso']);
                break;
            case 'Caso Trauma':
                $api->getCasoTrauma($data['Caso']);
                break;
            case 'Caso ExpG':
                $api->getCasoExpG($data['Caso']);
                break;
            case 'Caso ExpF':
                $api->getCasoExpF($data['Caso']);
                break;
            case 'Caso Medicamentos':
                $api->getCasoMedicamento($data['Caso']);
                break;
            case 'Caso Insumos':
                $api->getCasoInsumo($data['Caso']);
                break;
            case 'Caso Procedimientos':
                $api->getCasoProcedimiento($data['Caso']);
                break;
            case 'Caso Diagnosticos':
                $api->getCasoDiagnostico($data['Caso']);
                break;
            case 'Caso Obstetrico':
                $api->getCasoObstetrico($data['Caso']);
                break;
            case 'Medicaso':
                $api->getMedicaso($data['Caso']);
                break;
            case 'Pacicaso':
                $api->getPaciCaso($data['Caso']);
                break;
            default:
                # code...
                break;
